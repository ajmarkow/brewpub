...
class KegControl extends React.Component {
  ...
  handleUpdatingCurrentBrew = (id) => {
    const currentBrew = this.state.fullListOfBrews.filter(
      (brew) => brew.id === id
    )[0];
  ...
  removeBrew = (id) => {
    const newFullListofBrews = this.state.fullListOfBrews.filter(
      (keg) => keg.id !== id
    );
    this.setState({
  ...
  render() {
    ...
    if (this.state.currentBrew != null) {
      selectedVisibleState = (
        <KegDetail
          keg={this.state.currentBrew}
          onClickingDelete={this.removeBrew}
          onClickingDecrement={this.handleClickingDecrement}
          onClickingIncrement={this.handleClickingIncrement}
        />
      );
      ...
      selectedVisibleState = (
        <KegList
          kegList={this.state.fullListOfBrews}
          onBrewSelection={this.handleUpdatingCurrentBrew}
        />
      );
